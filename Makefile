# This Makefile is for the App::CSV extension to perl.
#
...
#     ABSTRACT => q[Manipulate CSV files from the command line]
#     AUTHOR => q[Gaal\ Yahas\ \<gaal\@forum2\.org\>]
#     DISTNAME => q[App-CSV]
#     EXE_FILES => [q[bin/csv]]
#     LICENSE => q[mit]
#     NAME => q[App::CSV]
#     PREREQ_PM => { MooseX::Getopt=>q[0], IO::String=>q[0], Text::CSV_XS=>q[0], Moose=>q[0] }
#     VERSION => q[0.01]
...
TO_INST_PM = lib/App/CSV.pm

PM_TO_BLIB = lib/App/CSV.pm \
	blib/lib/App/CSV.pm
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN1EXT) --perm_rw=$(PERM_RW) \
	  bin/csv $(INST_MAN1DIR)/csv.$(MAN1EXT) 
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) \
	  lib/App/CSV.pm $(INST_MAN3DIR)/App::CSV.$(MAN3EXT) 
...
$(INST_SCRIPT)/csv : bin/csv $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/csv
	$(CP) bin/csv $(INST_SCRIPT)/csv
	$(FIXIN) $(INST_SCRIPT)/csv
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/csv
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:         App-CSV' >> META_new.yml
...
ppd:
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Manipulate CSV files from the command line</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Gaal\ Yahas\ \&lt;gaal\@forum2\.org\&gt;</AUTHOR>' >> $(DISTNAME).ppd
	...
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="MooseX-Getopt" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' \
	  lib/App/CSV.pm blib/lib/App/CSV.pm 
